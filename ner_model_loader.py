...
if __name__ == '__main__':
    check_point_dir_path = "ner_model_cc/"
    ...
    xArray, yArray, lArray, segArray, sentArray = data_iterator(test_data, 20)#
    ...
    with tf.Session() as sess:
        ...
        for x, y, l, seg, sent in zip(xArray, yArray, lArray, segArray, sentArray):
            fetches = [crf_losess, crf_logits, crf_trans]
            feed_dict = dict()
            ...
            loss, logits, trans = sess.run(fetches, feed_dict)
            for logits_, y_, l_, char_ in zip(logits, y, l, sent):
                ...
                for pre_tag, gold_tag, test_char in zip(viterbi_sequence, y_, char_):
                    result_file_writer.write(str(test_char) + "\t" + str(id_to_tag[int(gold_tag)]) + "\t" + str(
                        id_to_tag[int(pre_tag)]) + "\n")
